...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $femaleBirdId = intval($_POST['female_bird_id'] ?? 0);
    ...
    if ($quantity <= 0 || empty($collectionDate)) {
        ...
        $db->query("UPDATE eggs SET female_bird_id = :bird, quantity = :qty, damaged_count = :damaged, collection_date = :date, notes = :notes WHERE id = :id");
        $db->bind(':bird', $femaleBirdId ?: null);
