...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($type) || empty($category) || $amount <= 0) {
        ...
        $db->query("UPDATE transactions SET transaction_type = :type, category = :category, amount = :amount, description = :description, transaction_date = :date WHERE id = :id");
        $db->bind(':type', $type);
        ...
        if ($db->execute()) {
            $message = 'مامەڵەکە بە سەرکەوتوویی نوێکرایەوە';
            $messageType = 'success';
            // Refresh data
            $db->query("SELECT * FROM transactions WHERE id = :id");
            $db->bind(':id', $id);
...
<div class="page-header">
    <div>
        <h2><i class="fas fa-edit"></i> دەستکاری مامەڵە</h2>
    ...
    <div class="col-lg-8">
        <div class="card">
            <div class="card-header">
                <i class="fas fa-exchange-alt"></i> دەستکاری مامەڵە
            </div>
            <div class="card-body">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="d-flex gap-3">
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="transaction_type" id="typeIncome" value="income" <?php echo $transaction['transaction_type'] == 'income' ? 'checked' : ''; ?> required>
                                    <label class="form-check-label text-success" for="typeIncome">
                                        <i class="fas fa-arrow-down"></i> داهات
                                    </label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="transaction_type" id="typeExpense" value="expense" <?php echo $transaction['transaction_type'] == 'expense' ? 'checked' : ''; ?>>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="category" class="form-select" id="categorySelect" required>
                                <option value="<?php echo $transaction['category']; ?>" selected><?php echo $transaction['category']; ?></option>
                            </select>
                        </div>
